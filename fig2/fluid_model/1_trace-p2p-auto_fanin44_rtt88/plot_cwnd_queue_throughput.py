...
# Make PDF/PS text editable (TrueType)
matplotlib.rcParams["pdf.fonttype"] = 42
matplotlib.rcParams["ps.fonttype"] = 42

# =========================
# Part 1: ns-3 Data Processing
# =========================

# Time offset in microseconds (ns-3 starts at 1s)
TIME_OFFSET_US = 1_000_000


def read_throughput_data(filename: str, time_offset_us: int = TIME_OFFSET_US):
    """
    Read ns-3 throughput data file.

    Expected format per line (tab-separated):
        <timestamp_us> <...> <bytes_per_interval> ...

    Returns:
        (time_us, throughput_gbps) as numpy arrays.
    """
    time_us = []
    throughput_gbps = []
    try:
        with open(filename, "r") as f:
            for line in f:
                ...
                if len(parts) >= 3:
                    ...
                    time_us.append(ts_us - time_offset_us)

                    # Throughput: (bytes * 8) / interval_seconds / 1e9
                    # Here interval = 100 us (per your original code)
                    thr = bytes_per_interval * 8.0 / (100e-6) / 1e9
                    throughput_gbps.append(thr)
    except FileNotFoundError:
        print(f"[WARN] File not found: '{filename}'. Skip plotting this series.")
        return np.array([]), np.array([])

    return np.array(time_us), np.array(throughput_gbps)


def read_cwnd_data(filename: str, time_offset_us: int = TIME_OFFSET_US):
    """
    Read ns-3 cwnd data file.

    Expected format per line (space-separated):
        <time_us> <cwnd_pkts>

    Returns:
        (time_us, cwnd) as numpy arrays.
    """
    time_us = []
    cwnd = []
    try:
        with open(filename, "r") as f:
            for line in f:
                ...
                time_us.append(t - time_offset_us)
                cwnd.append(c_size)
    except FileNotFoundError:
        print(f"[WARN] File not found: '{filename}'. Skip plotting this series.")
        return np.array([]), np.array([])

    return np.array(time_us), np.array(cwnd)


def read_queue_data(filename: str, time_offset_us: int = TIME_OFFSET_US):
    """
    Read ns-3 queue size data file.

    Expected format per line (space-separated):
        <time_us> <queue_size>

    Returns:
        (time_us, queue_size) as numpy arrays.
    """
    time_us = []
    queue_size = []
    try:
    ...
    return np.array(time_us), np.array(queue_size)
...
N_FLOWS = 2
F = np.array([4.0, 4.0])
D = np.array([0.903, 0.903])  # lags (in "model time")
C = 412.8
K = 80.0
N_PARAM = 4096 * 16
...
def dde_system_py(y, history_lookup, t, lags):
    ...
    # delayed ECN-like marking for each flow: p(t - lag_k) = 1{q_delayed > K}
    ...
    # delayed W for computing aggregation probability term in dq/dt
    ...
    for k in range(N_FLOWS):
        ...
        prod_term = max(0.0, prod_term)

        arrival_rate_k = (W_current[k] / R_current[k]) * (prod_term + F[k] * (1.0 - prod_term))
...
def solve_dde(dde_func, initial_history_vals, t_span, lags, dt=1.0):
    """
    Solve DDE with forward Euler and piecewise-constant history lookup.

    Args:
        dde_func: RHS function
        initial_history_vals: initial state for t<=0
        t_span: [t0, t1]
        lags: array-like of delays
        dt: step

    Returns:
        times, solution (shape: [len(times), n_vars])
    """
    times = np.arange(t_span[0], t_span[1], dt)
    n_vars = len(initial_history_vals)
    solution = np.zeros((len(times), n_vars))

    history_dict = {0.0: initial_history_vals.copy()}

    def history_lookup(tt):
        if tt <= 0.0:
            return initial_history_vals
        # nearest previous stored time (piecewise-constant)
        closest_time = max(k for k in history_dict.keys() if k <= tt)
    ...
    for i in range(1, len(times)):
        ...
        solution[i, :] = y_next
        history_dict[float(times[i])] = y_next

    return times, solution


# =========================
# Part 3: Main + Plotting
# =========================
...
def main():
    ...
    output_filename = "fanin44_rtt88_three_comparisons_combined.pdf"
