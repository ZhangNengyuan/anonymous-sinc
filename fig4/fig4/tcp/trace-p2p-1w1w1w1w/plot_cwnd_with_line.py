...
time_n0, cwnd_n0 = read_cwnd_data('n0-job1-cwnd-p2p-1w1w1w1w.txt')
time_m0, cwnd_m0 = read_cwnd_data('m0-job2-cwnd-p2p-1w1w1w1w.txt')
time_k0, cwnd_k0 = read_cwnd_data('k0-job3-cwnd-p2p-1w1w1w1w.txt')
time_l0, cwnd_l0 = read_cwnd_data('l0-job4-cwnd-p2p-1w1w1w1w.txt')

# Read queue data
time_q, queue_size = read_queue_data('n2-queueSize-p2p-1w1w1w1w.txt')

# Plot all data on single y-axis
ax1.plot(time_n0, cwnd_n0, label='flow1 (cwnd)', linewidth=1.5, color='#FF0000')
ax1.plot(time_m0, cwnd_m0, label='flow2 (cwnd)', linewidth=1, dashes=(10,10), color='#0000FF')
ax1.plot(time_k0, cwnd_k0, label='flow3 (cwnd)', linewidth=1.5, color='#FF8C00')
ax1.plot(time_l0, cwnd_l0, label='flow4 (cwnd)', linewidth=1, dashes=(10,10), color='#9932CC')
...
# Set x-axis range
max_time = max(np.max(time_n0), np.max(time_m0), np.max(time_k0), np.max(time_l0), 10000)
...
# Add vertical dashed lines at x=0 and x=3080
ax1.axvline(x=0, color='black', linestyle='--')
ax1.axvline(x=3080, color='black', linestyle='--')
ax1.grid(which='major', color='#D3D3D3', linestyle='--', linewidth=0.8)

# Add a double-headed arrow between the vertical lines
y_arrow_pos = max(np.max(cwnd_n0), np.max(cwnd_m0), np.max(cwnd_k0), np.max(cwnd_l0)) * 0.9
ax1.annotate('', xy=(0, y_arrow_pos), xytext=(3080, y_arrow_pos),
             arrowprops=dict(arrowstyle='<->', color='black', lw=1))
ax1.annotate('3080μs', xy=(3080/2, y_arrow_pos),
             ha='center', va='center',
...
plt.tight_layout()
plt.savefig('tcp_cwnd_comparison_4flow_line.pdf', dpi=300, bbox_inches='tight')
plt.close() 